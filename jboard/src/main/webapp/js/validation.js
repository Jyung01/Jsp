...
document.addEventListener('DOMContentLoaded', function(){
	...
	btnNick.addEventListener('click', async function(e){
		...
		if(data.count > 0){
			nickResult.innerText = '이미 사용중인 닉네임 입니다.';
			nickResult.style.color = 'red';
			isNickOk = false;
		}else{
			nickResult.innerText = '사용 가능한 닉네임 입니다.';
	...
	let preventDblClick = false; // 이중 클릭 방지
	...
	btnConfirm.addEventListener("click", async function(e) {
		...
		if (data.count > 0){
			emailResult.innerText = '인증 코드가 다릅니다.';
			emailResult.style.color = 'red';
			isEmailOk = false;
		} else {
	...
	inputHp.addEventListener("focusout", async function(e) {
		...
		if(data.count > 0){
			...
			hpResult.innerText = '사용하실 수 있는 휴대폰 입니다';
